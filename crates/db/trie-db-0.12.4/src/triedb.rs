    ...
    C: NodeCodec<H>,
{
    ...
    fn get_raw_or_lookup(
        ...
        match (
            partial_key == nibbleslice::EMPTY_ENCODED,
    ...
    C: NodeCodec<H>,
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if let Ok(node) = self
            .trie
            .get_raw_or_lookup(self.node_key, &self.partial_key)
        {
            match C::decode(&node) {
...
mod tests {
    ...
    fn iterator_seek() {
        ...
        assert_eq!(
            iter.next().unwrap().unwrap(),
            (b"A".to_vec(), DBValue::from_slice(b"A"))
        );
        iter.seek(b"!").unwrap();
