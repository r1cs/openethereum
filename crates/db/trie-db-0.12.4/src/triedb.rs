...
use super::{DBValue, Query, Result, Trie, TrieError, TrieItem, TrieIterator};
    ...
    C: NodeCodec<H>,
{
    ...
    pub fn new(
        db: &'db dyn HashDBRef<H, DBValue>, root: &'db H::Out,
    ...
    fn get_raw_or_lookup(
        &'db self, node: &[u8], partial_key: &[u8],
    ) -> Result<Cow<'db, DBValue>, H::Out, C::Error> {
    ...
    C: NodeCodec<H>,
{
    ...
    fn get_with<'a, 'key, Q: Query<H>>(
        &'a self, key: &'key [u8], query: Q,
    ) -> Result<Option<Q::Item>, H::Out, C::Error>
    where
        'a: 'key,
    {
        Lookup { db: self.db, query: query, hash: self.root.clone(), marker: PhantomData::<C> }
    ...
    C: NodeCodec<H>,
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if let Ok(node) = self.trie.get_raw_or_lookup(self.node_key, &self.partial_key) {
            match C::decode(&node) {
                Ok(Node::Leaf(slice, value)) => match (f.debug_struct("Node::Leaf"), self.index) {
                ...
                Ok(Node::Branch(ref nodes, ref value)) => {
                    let nodes: Vec<TrieAwareDebugNode<H, C>> = nodes
                        .into_iter()
                        .enumerate()
...
impl<'a, H: Hasher, C: NodeCodec<H>> TrieDBIterator<'a, H, C> {
    ...
    fn seek<'key>(
        &mut self, node_data: &DBValue, key: NibbleSlice<'key>,
                ...
                match node {
                    Node::Leaf(slice, _) => {
                        if slice >= partial {
                            ...
                            self.trail
                                .push(Crumb { status: Status::Exiting, node: node.clone().into() });
                        }
                    ...
                    Node::Extension(ref slice, ref item) => {
                        if partial.starts_with(slice) {
                            self.trail
                                .push(Crumb { status: Status::At, node: node.clone().into() });
                            self.key_nibbles.extend(slice.iter());
                            full_key_nibbles += slice.len();
                            partial = partial.mid(slice.len());
                            let data = self.db.get_raw_or_lookup(
                                &*item,
                                &key.encoded_leftmost(full_key_nibbles, false),
                            )?;
                            data
                        } else {
                    ...
                    Node::Branch(ref nodes, _) => match partial.is_empty() {
                        ...
                        false => {
                            ...
                            if let Some(ref child) = nodes[i as usize] {
                                let child = self.db.get_raw_or_lookup(
                                    &*child,
                                    &key.encoded_leftmost(full_key_nibbles, false),
                                )?;
                                child
    ...
    fn encoded_key(&self) -> ElasticArray36<u8> {
        ...
        if self.key_nibbles.len() % 2 == 1 {
            NibbleSlice::new_composed(
                &slice,
                &NibbleSlice::new_offset(&self.key_nibbles[(self.key_nibbles.len() - 1)..], 1),
            )
            .encoded(false)
        } else {
            slice.encoded(false)
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
                ...
                match (b.status.clone(), &b.node) {
                    ...
                    (Status::At, &OwnedNode::Extension(_, ref d)) => {
                        IterStep::Descend::<H::Out, C::Error>(
                            self.db.get_raw_or_lookup(&*d, &self.encoded_key()),
                        )
                    }
            ...
            match iter_step {
                ...
                IterStep::Descend::<H::Out, C::Error>(Err(e)) => return Some(Err(e)),
                IterStep::Continue => {}
            }
        }
    }
...
mod tests {
    ...
    use reference_trie::{NibbleSlice, RefLookup, RefTrieDB, RefTrieDBMut, Trie, TrieMut};
    use DBValue;
