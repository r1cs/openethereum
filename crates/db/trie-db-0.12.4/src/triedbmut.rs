                    ...
                    children
                        ...
                        .map(|(i, maybe_child)| {
                            maybe_child.map(|child| {
                                child_cb(
                                    child,
                                    &NibbleSlice::new_offset(&[i as u8], 1).encoded(false),
                                )
                            })
                        }),
                    value,
    ...
    fn insert_at(
        ...
        let (new_stored, changed) = self
            .inspect(stored, key, move |trie, stored, key| {
                trie.insert_inspector(stored, key, value, old_val)
                    .map(|a| a.into_action())
            })?
                ...
                if cp == existing_key.len() && cp == partial.len() {
                    ...
                    *old_val = Some(stored_value);
                    ...
                    let branch_action = self
                        .insert_inspector(branch, key, value, old_val)?
                        .unwrap_node();
                    ...
                    let augmented_low = self
                        .insert_inspector(low, key, value, old_val)?
                        .unwrap_node();
...
mod tests {
    ...
    fn insert_on_empty() {
        ...
        assert_eq!(
            *t.root(),
            ref_trie_root(vec![(vec![0x01u8, 0x23], vec![0x01u8, 0x23])])
        );
    }
    ...
    fn insert_replace_root() {
        ...
        assert_eq!(
            *t.root(),
            ref_trie_root(vec![(vec![0x01u8, 0x23], vec![0x23u8, 0x45])])
        );
    }
