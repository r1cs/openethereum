...
use super::node::{Node as EncodedNode, NodeKey};
use super::{DBValue, Result, TrieError, TrieMut};
use node_codec::NodeCodec;

use hash_db::{HashDB, Hasher};
use nibbleslice::{self, combine_encoded, NibbleSlice};
        ...
        + Copy,
{
    ...
    fn inline_or_hash<C, H>(
        node: &[u8], db: &dyn HashDB<H, DBValue>, storage: &mut NodeStorage<H::Out>,
    ) -> NodeHandle<H::Out>
    where
        C: NodeCodec<H>,
        H: Hasher<Out = O>,
    {
        C::try_decode_hash(&node).map(NodeHandle::Hash).unwrap_or_else(|| {
    ...
    fn from_encoded<C, H>(
        data: &[u8], db: &dyn HashDB<H, DBValue>, storage: &mut NodeStorage<H::Out>,
    ) -> Self
    where
        C: NodeCodec<H>,
        H: Hasher<Out = O>,
    {
        match C::decode(data).unwrap_or(EncodedNode::Empty) {
            ...
            EncodedNode::Extension(key, cb) => {
                Node::Extension(key.encoded(false), Self::inline_or_hash::<C, H>(cb, db, storage))
            }
            EncodedNode::Branch(ref encoded_children, val) => {
                let mut child = |i: usize| {
                    encoded_children[i].map(|data| Self::inline_or_hash::<C, H>(data, db, storage))
                };
                ...
                C::branch_node(
                    ...
                    children.iter_mut().map(Option::take).enumerate().map(|(i, maybe_child)| {
                        maybe_child.map(|child| {
                            child_cb(child, &NibbleSlice::new_offset(&[i as u8], 1).encoded(false))
                        })
                    }),
                    value,
                )
    ...
    C: NodeCodec<H>,
{
    ...
    pub fn from_existing(
        db: &'a mut dyn HashDB<H, DBValue>, root: &'a mut H::Out,
    ...
    fn cache(&mut self, hash: H::Out, _key: &[u8]) -> Result<StorageHandle, H::Out, C::Error> {
        let node_encoded =
    ...
    fn inspect<F>(
        &mut self, stored: Stored<H::Out>, key: &mut Partial, inspector: F,
    ...
    fn lookup<'x, 'key>(
        &'x self, mut partial: NibbleSlice<'key>, handle: &NodeHandle<H::Out>,
    ...
    fn insert_at(
        &mut self, handle: NodeHandle<H::Out>, key: &mut Partial, value: DBValue,
        old_val: &mut Option<DBValue>,
    ) -> Result<(StorageHandle, bool), H::Out, C::Error> {
        let h = match handle {
    ...
    fn insert_inspector(
        &mut self, node: Node<H::Out>, key: &mut Partial, value: DBValue,
        old_val: &mut Option<DBValue>,
                ...
                if partial.is_empty() {
                    ...
                    *old_val = stored_value;
                    ...
                    if let Some(child) = children[idx].take() {
                        ...
                        let leaf = self
                            .storage
                            .alloc(Stored::New(Node::Leaf(key.mid().encoded(true), value)));
    ...
    fn remove_at(
        &mut self, handle: NodeHandle<H::Out>, key: &mut Partial, old_val: &mut Option<DBValue>,
    ) -> Result<Option<(StorageHandle, bool)>, H::Out, C::Error> {
    ...
    fn remove_inspector(
        &mut self, node: Node<H::Out>, key: &mut Partial, old_val: &mut Option<DBValue>,
    ) -> Result<Action<H::Out>, H::Out, C::Error> {
        let partial = key.mid();
    ...
    pub fn commit(&mut self) {
        ...
        for (hash, _prefix) in self.death_row.drain() {
            self.db.remove(&hash);
        }
    ...
    fn commit_child(
        &mut self, handle: NodeHandle<H::Out>, prefix: &NodeKey,
    ) -> ChildReference<H::Out> {
        match handle {
...
mod tests {
    ...
    use reference_trie::{ref_trie_root, NodeCodec, RefTrieDBMut, ReferenceNodeCodec, TrieMut};
    use standardmap::*;
    use {env_logger, DBValue};

    fn populate_trie<'db>(
        db: &'db mut HashDB<KeccakHasher, DBValue>, root: &'db mut <KeccakHasher as Hasher>::Out,
        v: &[(Vec<u8>, Vec<u8>)],
    ) -> RefTrieDBMut<'db> {
    ...
    fn insert_value_into_branch_root() {
        ...
        assert_eq!(
            *t.root(),
            ref_trie_root(vec![(vec![], vec![0x0]), (vec![0x01u8, 0x23], vec![0x01u8, 0x23]),])
        );
    }
